...
temp_r <- lapply(temp_r, function(X) grep(paste0("^", X, "$"), rownames(eb_raw), ignore.case = TRUE))
temp_r <- which(rownames(eb_raw) %in% names(sapply(temp_r, function(X) which.min(apply(eb_raw[X, ], 1, function(Y) sum(Y > 0))))))
if (length(temp_r) > 0) { eb_raw <- eb_raw[-temp_r, ] }

# Filter out genes and cells with zero counts
eb_raw <- eb_raw[Matrix::rowSums(eb_raw) > 0, ]
eb_raw <- eb_raw[, Matrix::colSums(eb_raw) > 0]

# Calculate library size and number of detected genes per cell
cS <- data.frame(libSize = Matrix::colSums(eb_raw),
                 geneDetect = apply(eb_raw, 2, function(X) sum(X > 0)))

# Define p-values for filtering doublets and poor libraries
p_hi <- 1e-3
p_lo <- 1e-2

# Fit negative binomial distribution to library size
fitLibSize <- fitdistr(cS$libSize, "negative binomial")
c_hiL <- qnbinom(p_hi, size = fitLibSize$estimate["size"], mu = fitLibSize$estimate["mu"], lower.tail = FALSE)
c_loL <- qnbinom(p_lo, size = fitLibSize$estimate["size"], mu = fitLibSize$estimate["mu"], lower.tail = TRUE)

# Fit negative binomial distribution to number of detected genes
fitGeneDetect <- fitdistr(cS$geneDetect, "negative binomial")
c_hiG <- qnbinom(p_hi, size = fitGeneDetect$estimate["size"], mu = fitGeneDetect$estimate["mu"], lower.tail = FALSE)
c_loG <- qnbinom(p_lo, size = fitGeneDetect$estimate["size"], mu = fitGeneDetect$estimate["mu"], lower.tail = TRUE)

# Identify doublets and poor libraries
temp_doublets <- (cS$libSize > c_hiL) | (cS$geneDetect > c_hiG)
temp_crapLibs <- (cS$libSize < c_loL) | (cS$geneDetect < c_loG)

# Filter out doublets and poor libraries
eb_rawF <- eb_raw[, !(temp_doublets | temp_crapLibs)]
temp_postFgenes <- Matrix::rowSums(eb_rawF) > 0

# Create SingleCellExperiment object
eb1p <- SingleCellExperiment(assays = list(counts = as.matrix(eb_rawF)), 
                             rowData = list(feature_symbol = rownames(eb_rawF)), 
                             colData = list(barcode = colnames(eb_rawF)))

# Define mitochondrial gene prefix based on species
if (species == "human") {
  mitoGenePrefix <- "^MT-"
} else if (species == "mouse") {
  mitoGenePrefix <- "^mt-"
} else {
  stop("Unsupported species. Please check the mitochondrial gene prefix.")
}

# Identify mitochondrial genes
is_mito <- grep(mitoGenePrefix, rownames(eb1p), value = TRUE)

# Calculate per-cell QC metrics with mitochondrial genes as a subset
eb1p <- addPerCellQC(eb1p, subsets = list(Mt = is_mito))

# For reporting purposes
forText_mitoPct <- round(median(eb1p$subsets_Mt_percent, na.rm = TRUE))

# Define the threshold for dropping cells with high mitochondrial content
drop_mitoMads <- 4
drop_mito <- isOutlier(eb1p$subsets_Mt_percent, nmads = drop_mitoMads, type = "higher")

# Check the dimensions of eb1p and drop_mito
cat("Number of cells in eb1p:", ncol(eb1p), "\n")
cat("Length of drop_mito vector:", length(drop_mito), "\n")

# Ensure drop_mito has the correct length
if (length(drop_mito) != ncol(eb1p)) {
  stop("Length of drop_mito does not match the number of cells in eb1p.")
}
...
# Print summary
cat("Mitochondrial percentage threshold:", forText_mitoPct, "%\n")
cat("Number of cells dropped due to high mitochondrial content:", forText_dropMito, "\n")
cat("Number of MADs used for outlier detection:", forText_mitoMads, "\n")
cat("Number of genes lost after filtering:", forText_genesLost, "\n")
cat("Number of genes remaining:", forText_genesRemain, "\n")
...
cycDlibSize <- tapply(eb1$total,cycScores$phases,function(X) density(X))
#cycDgeneDetect <- tapply(eb1$total_features,cycScores$phases,function(X) density(X))
cycDgeneDetect <- tapply(eb1$detected,cycScores$phases,function(X) density(X))
...
plot(detected~total,data=colData(eb1),pch=21,cex=1.2,
     col=viridis(3,.5)[c(3,1,2)][cycScores$phases],
     bg=viridis(3,0.2)[c(3,1,2)][cycScores$phases],
...
#geneStatsR <- with(colData(eb1),data.frame(DR=detected/ncol(eb1),
#                                           MDTC=total/detected,
#                                           MTC=total/ncol(eb1),
#                                           sumTC=total))
...
# DR: Detection rate (fraction of cells in which the gene is detected)
# MDTC: Mean detected transcript count (mean count per gene across detected cells)
# MTC: Mean transcript count (mean count per gene across all cells)
# sumTC: Sum of transcript counts (total counts per gene)
geneStatsR <- data.frame(
  DR = rowSums(counts(eb1) > 0) / ncol(eb1),
  MDTC = rowSums(counts(eb1)) / rowSums(counts(eb1) > 0),
  MTC = rowSums(counts(eb1)) / ncol(eb1),
  sumTC = rowSums(counts(eb1))
)

# Calculate the maximum count per gene
geneStatsR$cellMax <- apply(counts(eb1), 1, max)
...
# Subset the SingleCellExperiment object to keep only the genes that are not low abundance
eb1F1 <- eb1[!drop_lowCell, ]
...
par(mar=c(3,3,0,0))
plot(subsets_Mt_percent~total,data=colData(eb1p),
     pch=21,cex=1.2,col=alpha("black",0.2),bg=alpha("black",0.1),
     xlab="Library Size",ylab="Mitochondrial Transcript Percent")
with(colData(eb1p),abline(h=median(subsets_Mt_percent)+drop_mitoMads*mad(subsets_Mt_percent),
                          lwd=2,lty=2,col=alpha("red",0.5)))
legend("topright",lty=2,lwd=2,col=alpha("red",0.5),bty="n",
       legend=paste(drop_mitoMads,"MADs above median"))
par(mar=c(0,3,0,0))
hist(eb1p$total,breaks=100,col="grey",main=NULL,xaxt="n")
par(mar=c(3,0,0,0))
barplot(hist(eb1p$subsets_Mt_percent,breaks=100,plot=F)$counts,
        horiz=T,space=0,col="grey",main=NULL,xlab="Frequency")
dev.off()
